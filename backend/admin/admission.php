 ...
 */
require_once __DIR__ . '/../config.php'; require_once __DIR__ . '/auth.php'; initSession(); if (!isAdmin()) { header('Location: login.php'); exit; } function h(?string $str): string { return htmlspecialchars($str ?? '', ENT_QUOTES, 'UTF-8'); } $db = getDB(); $modName = 'Admin'; $leadId = (int)($_GET['lead_id'] ?? 0); $currentTab = $_GET['tab'] ?? 'student_info'; $stmt = $db->prepare("SELECT * FROM leads WHERE id = ?"); $stmt->bind_param('i', $leadId); $stmt->execute(); $lead = $stmt->get_result()->fetch_assoc(); if (!$lead) { die("Lead not found."); } $db->query("INSERT IGNORE INTO lead_details (lead_id) VALUES ($leadId)"); if (isset($_POST['action']) && $_POST['action'] === 'upload_document') { $docName = (string)($_POST['doc_name'] ?? ''); $uploadError = ''; if ($docName && !empty($_FILES['document_file']['name']) && $_FILES['document_file']['error'] === UPLOAD_ERR_OK) { $file = $_FILES['document_file']; $tmpPath = $file['tmp_name']; $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION)); $allowed = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'pdf']; if (in_array($ext, $allowed)) { $filename = 'doc_' . $leadId . '_' . time() . '_' . rand(1000, 9999) . '.' . $ext; $uploadDir = __DIR__ . '/../uploads/'; if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true); $destPath = $uploadDir . $filename; if (move_uploaded_file($tmpPath, $destPath)) { $docUrl = '/backend/uploads/' . $filename; $stmt = $db->prepare("INSERT INTO lead_documents (lead_id, doc_name, doc_url) VALUES (?, ?, ?)"); $stmt->bind_param('iss', $leadId, $docName, $docUrl); if($stmt->execute()) { $_SESSION['upload_success'] = 'Document uploaded successfully.'; } } else { $_SESSION['upload_error'] = 'Failed to save file to server.'; } } else { $_SESSION['upload_error'] = 'Invalid file format. Allowed: jpg, png, pdf, etc.'; } } header("Location: admission.php?lead_id=$leadId&tab=documents"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'save_course') { $f_inst = (string)($_POST['selected_institute'] ?? ''); $f_fund = (string)($_POST['funding_type'] ?? ''); $f_exam = (string)($_POST['entrance_exam_name'] ?? ''); $f_score = (string)($_POST['entrance_score'] ?? ''); $upd = $db->prepare("UPDATE lead_details SET selected_institute=?, funding_type=?, entrance_exam_name=?, entrance_score=? WHERE lead_id=?"); $upd->bind_param('ssssi', $f_inst, $f_fund, $f_exam, $f_score, $leadId); if ($upd->execute()) { $_SESSION['upload_success'] = 'Course details updated successfully.'; } header("Location: admission.php?lead_id=$leadId&tab=course"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'add_academic') { $qual = (string)($_POST['qualification'] ?? ''); $inst = (string)($_POST['institution'] ?? ''); $year = (string)($_POST['passing_year'] ?? ''); $perc = (string)($_POST['percentage'] ?? ''); $stmt = $db->prepare("INSERT INTO lead_academic (lead_id, qualification, institution, passing_year, percentage) VALUES (?, ?, ?, ?, ?)"); $stmt->bind_param('issss', $leadId, $qual, $inst, $year, $perc); if ($stmt->execute()) { $_SESSION['upload_success'] = 'Academic qualification added.'; } else { $_SESSION['upload_error'] = 'Error adding academic qualification.'; } header("Location: admission.php?lead_id=$leadId&tab=academic"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'delete_academic') { $acadId = (int)($_POST['acad_id'] ?? 0); $db->query("DELETE FROM lead_academic WHERE id = $acadId AND lead_id = $leadId"); header("Location: admission.php?lead_id=$leadId&tab=academic"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'add_payment') { $amount = (float)($_POST['amount'] ?? 0); $date = (string)($_POST['payment_date'] ?? date('Y-m-d')); $mode = (string)($_POST['payment_mode'] ?? ''); $notes = (string)($_POST['notes'] ?? ''); $receipt = ''; $stmt = $db->prepare("INSERT INTO lead_payments (lead_id, amount, payment_date, payment_mode, notes) VALUES (?, ?, ?, ?, ?)"); $stmt->bind_param('idsss', $leadId, $amount, $date, $mode, $notes); if ($stmt->execute()) { $_SESSION['upload_success'] = 'Payment recorded successfully.'; } else { $_SESSION['upload_error'] = 'Error recording payment.'; } header("Location: admission.php?lead_id=$leadId&tab=payments"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'delete_payment') { $payId = (int)($_POST['pay_id'] ?? 0); $db->query("DELETE FROM lead_payments WHERE id = $payId AND lead_id = $leadId"); header("Location: admission.php?lead_id=$leadId&tab=payments"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'save_course') { $f_inst = (string)($_POST['selected_institute'] ?? ''); $f_fund = (string)($_POST['funding_type'] ?? ''); $f_exam = (string)($_POST['entrance_exam_name'] ?? ''); $f_score = (string)($_POST['entrance_score'] ?? ''); $upd = $db->prepare("UPDATE lead_details SET selected_institute=?, funding_type=?, entrance_exam_name=?, entrance_score=? WHERE lead_id=?"); $upd->bind_param('ssssi', $f_inst, $f_fund, $f_exam, $f_score, $leadId); if ($upd->execute()) { $_SESSION['upload_success'] = 'Course details updated successfully.'; } header("Location: admission.php?lead_id=$leadId&tab=course"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'add_academic') { $qual = (string)($_POST['qualification'] ?? ''); $inst = (string)($_POST['institution'] ?? ''); $year = (string)($_POST['passing_year'] ?? ''); $perc = (string)($_POST['percentage'] ?? ''); $stmt = $db->prepare("INSERT INTO lead_academic (lead_id, qualification, institution, passing_year, percentage) VALUES (?, ?, ?, ?, ?)"); $stmt->bind_param('issss', $leadId, $qual, $inst, $year, $perc); if ($stmt->execute()) { $_SESSION['upload_success'] = 'Academic qualification added.'; } else { $_SESSION['upload_error'] = 'Error adding academic qualification.'; } header("Location: admission.php?lead_id=$leadId&tab=academic"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'delete_academic') { $acadId = (int)($_POST['acad_id'] ?? 0); $db->query("DELETE FROM lead_academic WHERE id = $acadId AND lead_id = $leadId"); header("Location: admission.php?lead_id=$leadId&tab=academic"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'add_payment') { $amount = (float)($_POST['amount'] ?? 0); $date = (string)($_POST['payment_date'] ?? date('Y-m-d')); $mode = (string)($_POST['payment_mode'] ?? ''); $notes = (string)($_POST['notes'] ?? ''); $stmt = $db->prepare("INSERT INTO lead_payments (lead_id, amount, payment_date, payment_mode, notes) VALUES (?, ?, ?, ?, ?)"); $stmt->bind_param('idsss', $leadId, $amount, $date, $mode, $notes); if ($stmt->execute()) { $_SESSION['upload_success'] = 'Payment recorded successfully.'; } else { $_SESSION['upload_error'] = 'Error recording payment.'; } header("Location: admission.php?lead_id=$leadId&tab=payments"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'delete_payment') { $payId = (int)($_POST['pay_id'] ?? 0); $db->query("DELETE FROM lead_payments WHERE id = $payId AND lead_id = $leadId"); header("Location: admission.php?lead_id=$leadId&tab=payments"); exit; } if (isset($_POST['action']) && $_POST['action'] === 'delete_document') { $docId = (int)($_POST['doc_id'] ?? 0); $db->query("DELETE FROM lead_documents WHERE id = $docId AND lead_id = $leadId"); header("Location: admission.php?lead_id=$leadId&tab=documents"); exit; } $toast = ''; if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['action'])) { $f_father = (string)($_POST['father_name'] ?? ''); $f_mother = (string)($_POST['mother_name'] ?? ''); $f_gender = (string)($_POST['gender'] ?? 'Male'); $f_alt = (string)($_POST['alt_phone'] ?? ''); $f_dob = (string)($_POST['dob'] ?? '') ?: null; $f_nat = (string)($_POST['nationality'] ?? 'India'); $f_aadhar = (string)($_POST['aadhar_number'] ?? ''); $f_passport = (string)($_POST['passport_number'] ?? ''); $f_address = (string)($_POST['address'] ?? ''); $f_country = (string)($_POST['country_to_go'] ?? ''); if ($f_country) { $updLead = $db->prepare("UPDATE leads SET country=? WHERE id=?"); $updLead->bind_param('si', $f_country, $leadId); $updLead->execute(); } $f_inst = (string)($_POST['selected_institute'] ?? ''); $f_fund = (string)($_POST['funding_type'] ?? ''); $f_exam = (string)($_POST['entrance_exam_name'] ?? ''); $f_score = (string)($_POST['entrance_score'] ?? ''); $photoPath = (string)($_POST['existing_photo'] ?? ''); $uploadError = ''; if (!empty($_FILES['student_photo']['name']) && $_FILES['student_photo']['error'] === UPLOAD_ERR_OK) { $file = $_FILES['student_photo']; $tmpPath = $file['tmp_name']; $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION)); $allowed = ['jpg', 'jpeg', 'png', 'gif', 'webp']; if (in_array($ext, $allowed)) { $filename = 'photo_' . $leadId . '_' . time() . '_' . rand(1000, 9999) . '.' . $ext; $uploadDir = __DIR__ . '/../uploads/'; if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true); $destPath = $uploadDir . $filename; if (move_uploaded_file($tmpPath, $destPath)) { $photoPath = '/backend/uploads/' . $filename; } else { $_SESSION['upload_error'] = 'Failed to save photo to server.'; } } else { $_SESSION['upload_error'] = 'Invalid photo format. Use JPG, PNG, etc.'; } } $upd = $db->prepare("UPDATE lead_details SET 
        ...
        WHERE lead_id=?"); $upd->bind_param('ssssssssssssssi', $f_father, $f_mother, $f_gender, $f_alt, $f_dob, $f_nat, $f_aadhar, $f_passport, $f_address, $photoPath, $f_inst, $f_fund, $f_exam, $f_score, $leadId ); $toast = $upd->execute() ? "Student details updated successfully." : "Error: " . $db->error; header("Location: admission.php?lead_id=$leadId&saved=1"); exit; } $stmt = $db->prepare("SELECT * FROM lead_details WHERE lead_id = ?"); $stmt->bind_param('i', $leadId); $stmt->execute(); $det = $stmt->get_result()->fetch_assoc() ?? []; $d = [ 'father_name' => (string)($det['father_name'] ?? ''), 'mother_name' => (string)($det['mother_name'] ?? ''), 'gender' => (string)($det['gender'] ?? 'Male'), 'alt_phone' => (string)($det['alt_phone'] ?? ''), 'dob' => (string)($det['dob'] ?? ''), 'nationality' => (string)($det['nationality'] ?? 'India'), 'aadhar_number' => (string)($det['aadhar_number'] ?? ''), 'passport_number' => (string)($det['passport_number'] ?? ''), 'address' => (string)($det['address'] ?? ''), 'photo_path' => (string)($det['photo_path'] ?? ''), 'selected_institute' => (string)($det['selected_institute'] ?? ''), 'funding_type' => (string)($det['funding_type'] ?? 'Self Funded'), 'entrance_exam_name' => (string)($det['entrance_exam_name'] ?? 'NEET'), 'entrance_score' => (string)($det['entrance_score'] ?? ''), ]; $leadName = (string)($lead['name'] ?? ''); $leadPhone = (string)($lead['phone'] ?? ''); $leadEmail = (string)($lead['email'] ?? ''); $leadCountry = (string)($lead['country'] ?? ''); $leadCourse = (string)($lead['course'] ?? ''); $toast = isset($_GET['saved']) ? 'Student details updated successfully.' : ''; if(isset($_SESSION['upload_success'])){ $toast = $_SESSION['upload_success']; unset($_SESSION['upload_success']); } $uploadError = ''; if (isset($_SESSION['upload_error'])) { $uploadError = $_SESSION['upload_error']; unset($_SESSION['upload_error']); } if ($d['photo_path'] && strpos($d['photo_path'], '/public/uploads/') === 0) { $uploadError = 'Photo is stored locally and cannot be displayed. Please re-upload to fix.'; } $photoDisplay = $d['photo_path'] ?: 'https://ui-avatars.com/api/?name=' . urlencode($leadName) . '&size=200&background=6366f1&color=fff'; ?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="container">
  ...
  <?php if ($currentTab === 'payments'): ?>
    ...
    <?php  $totalPaid = array_sum(array_column($payments, 'amount')); ?>
    <div style="background:#fff; border:1px solid #e2e8f0; border-top:none; padding:20px;">
