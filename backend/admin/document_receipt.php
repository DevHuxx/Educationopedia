 ...
 */
require_once __DIR__ . '/../config.php'; require_once __DIR__ . '/auth.php'; initSession(); if (!isAdmin()) { header('Location: login.php'); exit; } $db = getDB(); $leadId = (int)($_GET['lead_id'] ?? 0); $stmt = $db->prepare("SELECT l.*, d.father_name, d.address, d.alt_phone, d.selected_institute FROM leads l LEFT JOIN lead_details d ON l.id = d.lead_id WHERE l.id = ?"); $stmt->bind_param('i', $leadId); $stmt->execute(); $lead = $stmt->get_result()->fetch_assoc(); if (!$lead) die("Lead not found."); $docs = $db->query("SELECT * FROM lead_documents WHERE lead_id = $leadId ORDER BY id ASC")->fetch_all(MYSQLI_ASSOC); ?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
