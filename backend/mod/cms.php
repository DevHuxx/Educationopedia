...
if (!empty($modRights['site_content_rights'])) {
    array_push($allowedSections, 'content', 'images', 'testimonials', 'blog', 'footer', 'stats', 'gallery');
}
if (!empty($modRights['exam_rights'])) $allowedSections[] = 'exams';

if (empty($currentSection)) {
    if (!empty($allowedSections)) {
        $currentSection = $allowedSections[0];
    } else {
        die("<h2 style='font-family:sans-serif;padding:20px;color:white;'>You do not have any CMS permissions assigned.</h2>");
    }
}

if (!in_array($currentSection, $allowedSections)) {
    die("<h2 style='font-family:sans-serif;padding:20px;color:white;'>You do not have permission to view this section.</h2>");
}
?>
<!DOCTYPE html>
<html lang="en">
...
async function toggleModRight(id, rightField) {
    ...
    try {
        const res = await fetch('../admin/api_admin.php?action=toggle_mod_right', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
<div id="uniDetailsModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:9999;overflow:auto;padding:30px 16px;" onclick="if(event.target===this)closeUniModal()">
    ...
    <a href="index.php" style="color:#94a3b8; font-size:13px; font-weight:bold; margin-bottom:10px; display:inline-block;">🔙 Back to Leads CRM</a>
    <?php if (!empty($modRights['university_rights'])): ?>
        <a href="?section=universities" class="<?= $currentSection==='universities'?'active':'' ?>">🏛️ Universities</a>
    <?php endif; ?>
    <?php if (!empty($modRights['site_content_rights'])): ?>
        <a href="?section=content" class="<?= $currentSection==='content'?'active':'' ?>">📝 Site Content</a>
        <a href="?section=gallery" class="<?= $currentSection==='gallery'?'active':'' ?>">🖼️ Gallery</a>
        <a href="?section=footer" class="<?= $currentSection==='footer'?'active':'' ?>">⚙️ Footer & Contact</a>
    <?php endif; ?>
    <?php if (!empty($modRights['exam_rights'])): ?>
        <a href="manage_exams.php" class="<?= $currentSection==='exams'?'active':'' ?>">🎓 Exams</a>
    <?php endif; ?>
</nav>
require_once __DIR__ . '/../config.php';
require_once __DIR__ . '/auth_mod.php';
